...
def find_similar_in_face_list(subscription_key):
    ...
    for similar_result in similar_results:
        persisted_faces = [pf for pf in persisted_faces if pf.persisted_face_id == similar_result.persisted_face_id]
        if not persisted_faces:
...
def find_similar_in_large_face_list(subscription_key):
    ...
    training_status = face_client.large_face_list.get_training_status(large_face_list_id=large_face_list_id)
    ...
    persisted_faces = face_client.large_face_list.list_faces(large_face_list_id)
    if not persisted_faces:
    ...
    similar_results = face_client.face.find_similar(face_id=detected_faces[0].face_id, large_face_list_id=large_face_list_id)
...
def group_run(subscription_key):
    ...
    for image_file_name in image_file_names:
        ...
        detected_faces = _detect_faces_helper(face_client=face_client, image_url=image_url_prefix + image_file_name)
...
def identify_in_person_group(subscription_key):
    ...
    for identify_result in identify_results:
        person = face_client.person_group_person.get(person_group_id=person_group_id, person_id=identify_result.candidates[0].person_id)
...
def verify_face_to_face(subscription_key):
    ...
    detected_faces1 = _detect_faces_helper(face_client=face_client, image_url=image_url_prefix + source_image_file_name1)
    source_face_id1 = detected_faces1[0].face_id

    # Detect faces from source image file 2.
    detected_faces2 = _detect_faces_helper(face_client=face_client, image_url=image_url_prefix + source_image_file_name2)
...
def verify_in_person_group(subscription_key):
    ...
    person_id = face_client.person_group_person.create(person_group_id=person_group_id, name="Dad").person_id
...
def verify_in_large_person_group(subscription_key):
    ...
    person_id = face_client.large_person_group_person.create(large_person_group_id=large_person_group_id, name="Dad").person_id
    ...
    # Verification example for faces of the same person.
    verify_result = face_client.face.verify_face_to_person(
        face_id=_detect_faces_helper(face_client=face_client, image_url=image_url_prefix + source_image_file_name1)[0].face_id,
        person_id=p.person_id, 
        large_person_group_id=large_person_group_id
    )
    if verify_result.is_identical:
